//baaal mainsher code copy koirao pari na
...
ll chalok(string s,ll cnt,ll sz,ll x){
	ll num1=0;
	ll num2=0;
	ll ssize=s.size();
	string s1;
	...
	while(cnt<=x){
		...
		ll tod=((ssize-cnt)%mod+mod)%mod;
		ssize=(ssize+((tod%mod)*((num1)%mod))%mod)%mod;
		ll cur=s.size();
		cnt++;
			
			while(s.size()<x && num1--){
				for(int i=cnt-1;i<cur;i++){
					s+=s[i];
				}
			}
