...
pi  loc[1000007];
int dp[1000007];
int solve(int index,int x,int y,int ct){
	if(index>=n){
		return 0;
	}
	if(dp[ct]!=-1){
	...
	int d=abs(x-loc[index].ff)+abs(y-loc[index].ss);
	int s1=0;
	int s2=0;
	int d2=cel[index]-ct;
	
	if(d<=d2){
		s1=1+solve(index+1,loc[index].ff,loc[index].ss,cel[index]);
	}
	s2=solve(index+1,x,y,ct);
	dp[ct]=max(s1,s2);
	return dp[ct];
}
