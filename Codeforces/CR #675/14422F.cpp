...
ll lcm(ll a,ll b){
    ll g=gcd(a,b);
    return (((a%mod * b%mod)%mod)* binaryexp(g,mod-2))%mod;
}
...
ll query(ll node,ll s,ll e,ll le,ll ri){
	...
	if(le<=s && ri>=e){
     ...
     ll mid1=(s+e)/2;
    ll d1=query(node*2, s,mid1 , le,ri );
         ll d2= query(2*node+1, mid1+1, e, le, ri);
         return lcm(d1,d2)%mod;
}
...
int main(){
 	...
 	for(int i=0;i<q;i++){
 		...
 		ll le=min(range1,range2);
 		ll ri=max(range1,range2);
 		//cout<<"asdasd"<<le<<" "<<ri<<endl;
 		ll ans =query(1,1,n,le,ri);
